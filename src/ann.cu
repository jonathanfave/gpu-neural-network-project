...
{
    matrix_t *z1 = alloc_matrix(nn->max_neurons_in_layer, nn->minibatch_size);
    matrix_t *z2 = alloc_matrix(nn->max_neurons_in_layer, nn->minibatch_size);
    ...
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        matrixDotCUDAManaged2(nn->layers[l]->weights->m, nn->layers[l-1]->activations->m, z1->m, nn->layers[l]->weights->rows, nn->layers[l-1]->activations->rows, z1->columns); // z1 <- w^l x a^(l-1)
        matrixDotCUDAManaged2(nn->layers[l]->biases->m, one->m, z2->m, nn->layers[l]->biases->rows, one->rows, z2->columns); // z2 <- b^l x 1        
...
void forward2(ann_t *nn, double (*activation_function)(double)) {
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        matrixDotCUDAManaged3(nn->layers[l]->weights, nn->layers[l-1]->activations, z1); // z1 <- w^l x a^(l-1)
        matrixDotCUDAManaged3(nn->layers[l]->biases, one, z2); // z2 <- b^l x 1        
        matrix_sum(z1, z2, nn->layers[l]->z); // z^l <- z1 + z2 <=> z^l <- w^l x a^(l-1) + b^l x 1      
...
void backward(ann_t *nn, matrix_t *y, double (*derivative_actfunct)(double))
{
    ...
    for (int l = L; l > 1; l--)
    {
        ...
        matrixDotCUDAManaged2(tw->m, nn->layers[l]->delta->m, delta_tmp->m, tw->rows, nn->layers[l]->delta->rows, delta_tmp->columns); // (w^l)T x delta^l
    ...
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        matrixDotCUDAManaged2(nn->layers[l]->delta->m, nn->layers[l]->delta->m, ta->m, nn->layers[l]->delta->rows, nn->layers[l]->delta->rows, ta->columns); // w1 <- delta^l x (a^(l-1))^T
        ...
        matrixDotCUDAManaged2(nn->layers[l]->delta->m, one->m, b1->m, nn->layers[l]->delta->rows, one->rows, b1->columns); // b1 <- delta^l x 1^T
